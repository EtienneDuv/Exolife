...
function img_out = filtreDerive(matrice)
    ...
    table = zeros(SizeX, SizeY);
    
    for i = 2 : SizeX-1,
        for j = 2 : SizeY-1,
            // Utilisation de la formule sqrt(Ii²+Ij²) et avec les pixels Ii et Ij obtenus à l'aide de la matrice de dérivé.
            
            table(i, j) = round(sqrt( round(.5*(-matrice(i-1, j)+matrice(i+1, j)))*round(.5*(-matrice(i-1, j)+matrice(i+1, j))) + round(.5*(-matrice(i, j-1)+matrice(i, j+1)))*round(.5*(-matrice(i, j-1)+matrice(i, j+1)))));
        end
    end
